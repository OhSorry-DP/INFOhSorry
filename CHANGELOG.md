...
- mDNS 가 `v3.ohsorry.local` 질의에도 같은 LAN IP 로 응답한다.
- 🔴 **v3 는 원격모드를 아직 지원하지 않는다** — 본인 카드가 `/api/me` 실시간이 아니라 CDN 데이터이고,
  로그인·`/api/me/*` POST 는 프록시가 정적 GET 만 받아오므로 동작하지 않는다.
- ⚠️ 점이 두 개인 `.local` 이름은 일부 안드로이드에서 풀리지 않을 수 있다.
...
- 사용법: DevTools 콘솔에서 `await syncScores()`(선택 계정) 또는 `await syncScores('C…')`. SQL 은 클립보드에 복사를 시도한다.
- 삭제 대상 셋: `ac-on-inf`(INF 계정에 `played_version > 0`) · `noplay-but-db`(tsv 는 NO PLAY 인데 서버 행 존재) ·
  `exceeds-tsv`(서버 행의 점수 **또는** 램프가 tsv 초과 — 그 `score_id` 만).
- 🔴 **tsv 에 행 자체가 없는 차트는 지우지 않는다** — 미해금/읽기 실패와 구별할 수 없다.
- 🔴 **`played_version = -10`(DBR 배틀)은 비교·삭제에서 제외한다** — INF 계정에 정상적으로 존재하는 기록이다.
- `exceeds-tsv` 근거: tsv 의 점수와 램프는 게임이 기억하는 **역대 최고값**이라, 과거 날짜 행이 이를 넘을 수 없다.
  ⇒ 날짜로 따로 가를 필요 없이 초과 행만 지우면 정상 과거 이력은 남는다.
- 왜 삭제가 필요한가: `upsert_scores` 가 best-only 라 정상 기록을 다시 올려도 잘못 올라간 높은 값이 내려가지 않는다.
- 안전장치: 서버 조회가 한 페이지라도 실패하면 중단 · tsv 가 비었거나 플레이 기록 0 이면 중단 ·
  tsv 판정 삭제분이 INF 행의 30%(`MAX_DELETE_RATIO`)를 넘으면 SQL 을 만들지 않고 경고(`{ force: true }` 로 우회) ·
  SQL 에 `iidx_id` 조건을 같이 걸어 잘못 붙여넣어도 다른 계정 행은 지워지지 않는다.
- 🔴 **섞인 tsv 를 정본으로 돌리면 섞인 기록이 정상으로 판정된다.** 섞인 계정 폴더를 지우고 게임으로 tsv 를 새로 만든 뒤 실행할 것.
- 삭제 후에는 게임을 끈 상태에서 수동 업로드로 정상 기록을 다시 올린다. 별값(★)은 단조 래칫이라 이 정리로 내려가지 않는다.
- anon key 에는 DELETE 권한이 없고, 삭제 RPC 를 anon 에 열면 공개된 키로 누구나 남의 기록을 지울 수 있어 SQL 생성까지만 한다.

### v0.0.126 — 2026-09-23 계정 전환 시 기록이 섞이던 버그 수정 (ID 축 가드 복원)
...
`lo` 를 열고(`null`) `hi` 를 `AAA_TARGET_HI`(0.25)로 낮췄으며, AAA 만 「적정 난이도 근접순」으로 정렬한다.

- 🔴 **`lo: null` 과 `sortTarget` 은 세트다** — 정렬을 종전(`targetStar` 오름차순)으로 되돌리면
  목록이 「안 친 쉬운 곡」으로 덮인다. 둘 중 하나만 옮기지 말 것.
- ⚠️ 두 상수는 **현행 자에서 직접 잰 값이라 `×S` 를 곱하지 않는다**(`maxm` 예외와 같은 취급).
- 미플레이(`rate == null`)는 **종전대로 제외**한다 — 2026-08-29 사용자 확정.
- `a`·`aa`·`maxm` 은 무변경. 파일 상단 주의문대로 **웹 정본과 상수를 1:1 로 유지**해야 한다.
...
- `tracker.tsv` 가 바뀐 뒤 45초 동안 추가 변경이 없으면 Supabase 업로드를 트리거합니다. 최악 지연이 약 11분에서 약 4.5분으로 줄어듭니다.
- 마지막으로 **성공한** 업로드에서 3분이 지나지 않았으면 그 시점까지 미뤘다가 올립니다 — 연속 플레이 중 업로드가 촘촘히 쏘아지는 것을 막는 쿨다운입니다.
- 첫 업로드 전에는 이 경로가 동작하지 않습니다. 초기 업로드는 종전대로 감지 후 3분 스케줄(`INITIAL_UPLOAD_DELAY_MS`)이 담당하고, 디바운스는 그 뒤부터 동작합니다.
- 기존 10분 주기(`STAR_REFRESH_INTERVAL_MS`)는 TSV 변경 이벤트를 놓치는 경우의 fail-safe 로 유지합니다. 종료 시 마지막 1회 업로드·수동 업로드 버튼(5분 쿨다운)·원격모드 SSE 실시간 푸시는 변경하지 않았습니다.
- 스냅샷 캡처용 400ms 디바운스(`tsvChangedDebounceRef`)와는 별개 타이머라, 화면 반영과 원격 카드 갱신은 종전처럼 플레이 즉시입니다.
- 문서 동기화 — `docs/data-flow.md` · `docs/architecture.md` · `docs/README.md` · `docs/sp.md` 의 업로드 타이밍 표기 갱신.

### v0.0.124 — 2026-09-17 진단 로그 파일 저장

`스냅샷 보류` 같은 진단 로그가 화면 로그 패널의 30줄 링버퍼에만 남아 앱을 끄면 사라졌다. 같은 증상으로 v0.0.120~123 을 네 번 릴리즈하는 동안 실패 사유를 한 번도 확보하지 못한 원인이 이것이다. 동작을 바꾸는 수정은 하지 않았고, 진단 근거를 남기는 것만 추가했다.

- 진단 로그를 `%APPDATA%/ohSorryScoreINF/logs/diag.log` 에 시각과 함께 기록합니다. 2MB 를 넘으면 `diag.prev.log` 로 1회 회전합니다.
- Reflux 로그 패널 하단에 로그 파일 경로를 표시합니다.
- 파일 기록 실패는 앱 동작에 영향을 주지 않습니다.

### v0.0.123 — 2026-09-13 스냅샷 보류 사유 표시 및 세션별 재시도

- `스냅샷 보류` 진단 로그에 게임 프로세스·메모리 읽기 오류·IIDX ID 상태와 세션 정보를 표시하고, 같은 문구의 반복 로그를 억제합니다.
- 옛 세션의 스냅샷이 남아 있어도 현재 세션 generation이 다르면 스냅샷 확보 재시도를 수행합니다. 업로드 신원 검증 조건은 변경하지 않았습니다.
...
- **원인**: `src/main/reflux.ts` 의 `watchTsv()` 가 tracker.tsv 변경 시각을 `st.mtime.getTime()`(정수 밀리초)으로 캡처해 `tsvChanged` 이벤트에 실어 보냈는데, `src/main/accountTsvStore.ts` 의 `snapshotTsv()` 는 승인 직전 새로 뜬 `fsp.stat().mtimeMs`(NTFS 100ns 단위라 소수점 있는 float)와 이를 직접 비교했다. 정수와 소수를 비교하니 파일이 전혀 안 바뀌어도 항상 다르게 나와 매번 거부됐다.
- **수정**: `watchTsv()` 도 `st.mtimeMs` 를 그대로 사용하도록 통일. 다른 소비처(`account:readTsv`, 화면 표시용 `tsvMtime`)는 이미 전부 `mtimeMs` 를 쓰고 있어 이 한 곳만 어긋나 있었다.
- v0.0.117 에서 추가한 "스냅샷 거부: tracker.tsv 가 쓰는 도중이라 건너뜀" 진단 로그가 이 버그를 정확히 가리켜 원인 확정에 결정적이었다.
- 🔴 실기 검증 없이 릴리즈. 이번에도 typecheck/build 만 통과.
...
- **업로드 직전 IIDX ID 재검증** — 메모리 값과 캐시 프로필이 다르면 업로드를 막습니다. 단, 게임 종료 후 실행되는 `final` 업로드는 메모리 재읽기 실패 시 캐시값으로 마지막 결과를 보존합니다.
- **메모리 IPC 견고성** — 프로세스 미존재를 기계 판독 가능한 플래그로 전달하고, 프로세스 핸들 획득 중 예외를 정상 IPC 실패 응답으로 처리합니다.
- ⚠️ **재검증 자체가 예외를 흘리지 않도록 차단** — `readIidxIdFresh()` 가 reject 되면 `tryUpload` 가 통째로 실패해 `final` 트리거에서 `upload.finalDone()` 까지 도달하지 못하고(메인이 6초 타임아웃까지 대기) **마지막 플레이 업로드가 유실**됩니다. 내부에서 잡아 `ok:false` 로만 표현해 설계대로 fail-open 되게 했습니다.
- 메모리 스캔 핸들러 3곳의 지역변수 `process` → `proc` (Node 전역 `process` 섀도잉 제거 — 지금은 무해하지만 그 블록에 `process.env` 한 줄만 들어가도 조용히 깨집니다).
- 검증: `npm run typecheck`, `npm run build`.
...
- 확인법: 게임 껐다 켠 뒤 INF devtools 콘솔에서 `[setUser진단]` 메시지 1줄 확인.

### v0.0.82 — 2026-06-14 원격모드 루트 자동 진입 (IP:3000 → /osr/?remote)
- `http-server.ts`: 루트(`GET /`) 접속을 `/osr/?remote` 로 302 리다이렉트. 폰/PC2 에서 `http://PC-IP:3000` 만 쳐도 바로 오소리웹 원격 카드(본인 실시간, `/api/me`)가 뜬다. (이전엔 `/osr/?remote` 전체 경로를 직접 입력해야 함 — v0.0.77.) INF 자체 renderer 원격제어 화면이 필요하면 `/index.html` 로 직접 접근.
