...
| [data-flow.md](data-flow.md) | 외부 데이터 소스(ereter/zasa/rating/spTier) gist 캐시 + TTL, `recommendCore` 의 gist 코어 통합, tsv 실시간 reload + Supabase 업로드(감지 후 3분 → 15분 주기 + 종료 시 1회) 분리, http-server LAN 원격제어(SSE)+연결(ohsorry.local/QR), IIDX ID 전환 가드 |
...
| [sp.md](sp.md) | **SP(싱글) 데이터 흐름** — Reflux 메모리 SP 5 slot → ① Supabase `play_style:0` 적재(SP10~12, 15분 주기), ② 원격모드 `/api/me` `sp_charts_json`+`sp_tier12` 실시간(SSE·setUser dedup·/osr 네트워크우선) |
