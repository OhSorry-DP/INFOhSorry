...
export const ipcHandlers: Record<string, (...args: never[]) => unknown> = {
  ...
  'memory:scan': async (...args: never[]) => {
    ...
    let found: ReturnType<typeof findInfinitas> = null;
    try {
      ...
      if (found) closeHandle(found.handle);
    }
  },
  ...
  'memory:read-via-anchor': async (...args: never[]) => {
    ...
    try {
      found = findInfinitas(exeName);
      if (!found) return { ok: false, error: '프로세스 못 찾음', processMissing: true };
  ...
  //   갱신될 때마다 SSE 로 PC2(오소리웹 ?remote)에 me:update 알림 → 보고 있는 본인 카드 조용히 재렌더.
